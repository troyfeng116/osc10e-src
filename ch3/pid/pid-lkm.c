...
static ssize_t proc_read(struct file *file, char __user *usr_buf, size_t count, loff_t *pos)
{
     ...
     * the kernel module function associated with reading from /proc/pid should return 0.
     */
    if (tsk == NULL)
    {
        printk(KERN_INFO "Unable to find task with pid %ld\n", l_pid);
        return 0;
    }
...
static ssize_t proc_write(struct file *file, const char __user *usr_buf, size_t count, loff_t *pos)
{
    ...
    if (!sscanf(k_mem, "%ld", &l_pid))
    {
        printk(KERN_INFO "Error copying from user\n");
        return -1;
    }

    printk(KERN_INFO "Read long pid value %ld from user\n", l_pid);
